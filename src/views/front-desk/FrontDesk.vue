  ...
  >
    ...
    >
      <div class="flex-1 flex items-center justify-end gap-[12px]">
        <div
          class="flex rounded overflow-hidden border border-[var(--color-border)] text-sm"
        >
          <button
            class="w-[120px] py-[6px] font-medium"
            :class="
              locale === 'zh'
            ...
            "
            @click="handleSwitchLanguage('zh')"
          >
            {{ t("toggle.zh") }}
          </button>
          <button
            class="w-[120px] py-[6px]"
            :class="
              locale === 'en'
                ? 'bg-[var(--color-lang-active-bg)] text-[var(--color-lang-active-text)]'
                : 'bg-[var(--color-lang-inactive-bg)] text-[var(--color-lang-inactive-text)]'
        ...
        <div class="flex items-center gap-[12px]">
          ...
          <div
            ...
            :class="isDark ? 'bg-gray-900' : 'bg-gray-300'"
            @click="toggleTheme"
          >
            <span
              class="text-xs font-bold transition-all duration-300 absolute"
              :class="
                isDark ? 'text-white left-2.5' : 'text-gray-600 right-2.5'
              "
            >
            </span>
            <div
              class="w-5 h-5 bg-white rounded-full shadow-md absolute transition-all duration-300"
      ...
      <div class="max-w-[1280px] max-h-[720px] flex gap-[16px]">
        ...
        >
          <div class="flex-1 min-h-0 grid place-items-center overflow-hidden">
            ...
            >
              <div
                v-for="item of placedItems"
                :key="item.id"
                class="absolute overflow-hidden rounded-lg cursor-pointer"
                :style="{
                  left: item.x + 'px',
                  top: item.y + 'px',
                  width: item.width + 'px',
        ...
        >
          <div v-if="selectedItem">
            ...
            <div class="mt-[20px] flex flex-col gap-[12px]">
              <p class="text-base text-[var(--color-text-primary)]">
                {{ t("info.name") }}: {{ selectedItem.name }}
              </p>
              <p class="text-base text-[var(--color-text-primary)]">
                {{ t("info.description") }}: {{ selectedItem.description }}
              </p>
            </div>
          </div>
        </aside>
